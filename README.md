...
- **Path traversal protection**: browse paths are canonicalized and validated against the root directory. Transfer destinations are NOT validated — the receiver trusts the peer (see [wiki/protocol.md](wiki/protocol.md) for the trust model: drift is for trusted peers on a monitored network, encryption protects the wire not the endpoint)
